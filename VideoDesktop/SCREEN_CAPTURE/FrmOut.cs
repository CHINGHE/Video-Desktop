        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (m_bMouseEnter || m_bLoad)
            {
                using (SolidBrush sb = new SolidBrush(Color.FromArgb(150, 0, 0, 0)))
                {
                    ...
                    string strDraw = "原始尺寸:\t[" + m_bmp.Width + "," + m_bmp.Height + "]"
                        + "\t比例:" + ((double)(this.Width - 2) / m_bmp.Width).ToString("F2") + "[W]"
                        + "\r\n当前尺寸:\t[" + (this.Width - 2) + "," + (this.Height - 2) + "]"
                        + "\t比例:" + ((double)(this.Height - 2) / m_bmp.Height).ToString("F2") + "[H]";
                    ...
                    g.FillRectangle(sb, rectString);
                    g.DrawString("移动 [W,S,A,D]  调整范围[T,G,F,H]\r\n缩放 [鼠标滚轮] 退出 [鼠标右键]", this.Font, Brushes.Wheat, rectString, sf);
                    g.DrawString("保存原始\r\n保存当前", this.Font, Brushes.Wheat, rectString.X + 12, rectString.Y);
